...
# ### Particle Filter for ECE470 Fall 2019
...
class particleFilter:
    ...
    def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
        ...
        self.particles = [[0 for i in range(2) ] for j in range (self.numParticles)]
        self.weights = [0 for i in range(self.numParticles)]
        randomX = np.random.uniform(0, self.curMax[0], self.numParticles)
        randomY = np.random.uniform(0, self.curMax[1], self.numParticles)
        
        for i in range(len(randomX)):
            self.particles[i] = [randomX[i], randomY[i]]
    ...
    def Sample_Motion_Model(self, u_t=0):
        ...
        for i in range(len(self.particles)):
            newPosition = self.model.simulateNextPosition(self.particles[i],u_t)
            self.particles[i] = newPosition
    ...
    def Measurement_Model(self):
        ...
        newWeights = [0 for i in range(self.numParticles)]
        robotsPosM = self.model.readingSensor()
        for i in range(len(self.particles)):
            
            partPosM = self.model.readingMap(self.particles[i])
            #Calc Distance 
            a = np.array(robotsPosM)
            b = np.array(partPosM)
            dist = np.linalg.norm(a-b)
            
            #Calc Weight and update
            weight = np.exp((-(dist**2))/(2*self.std))
            newWeights[i] = weight
            
        
        sumWeights = sum(newWeights)
    ...
    def calcPosition(self):
        ...
        for i in range(len(self.weights)):
            particleCoord = self.particles[i]
            particleXCoord = particleCoord[0]
            particleYCoord = particleCoord[1]
            
            estimateX = estimateX + (particleXCoord*self.weights[i])
            estimateY = estimateY + (particleYCoord*self.weights[i])
            
        
        if (estimateX < self.curMin[0]):
            estimateX = self.curMin[0]
        elif (estimateX > self.curMax[0]):
            estimateX = self.curMax[0]
                
        if (estimateY < self.curMin[1]):
            estimateY = self.curMin[1]
        elif (estimateY > self.curMax[1]):
            estimateY = self.curMax[1]
    ...
    def resampling(self):
        ...
        """
        The code below creates a continues probablitly distribution.
        
        """
        newParticles = []
        N = len(self.particles)
        #Create pdf array for the weights 
        cumulative_sum = np.cumsum(self.weights)
        #ensure the array ends in 1
        cumulative_sum[-1] = 1
        ...
        for i in range(N):
            ...
            #find the location where the number is located in sorted list 
...
#         ## TODO: Comment the code below:
        # """
       #  The code below uses a Roulette Wheel implmentation that by definintion incorporates randomness. 
        
        # """
          #Initialize array and number of particles 
#         newParticles = []
#         N = len(self.particles)

          #index chosen randomly based on quantity of particles
#         index = int(np.random.random() * N)

          #Beta initialzed to zero and max weight quantity mw
#         beta = 0
#         mw = np.max(self.weights)

          #Iterate for each particle 
#         for i in range(N):
              #Assign beta based on mw and a randomness incorporated by random()
#             beta += np.random.random() * 2.0 * mw
              #re-iterate until beta is smaller than the weight at the calculated index
#             while beta > self.weights[index]:
                  #decrease beta until it occurs
#                 beta -= self.weights[index]
                  #update index
#                 index = (index + 1) % N

              #once satisfied append new particle at the end of newParticles array
#             newParticles.append(self.particles[index])
#         self.particles = newParticles    
